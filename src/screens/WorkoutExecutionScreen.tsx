...
export default function WorkoutExecutionScreen({ workout, onFinish }: Props) {
  ...
  const handleCompleteSet = () => {
    ...
    if (currentSet < exercise.sets.length - 1) {
      ...
      Alert.alert(
        'Parabens!',
        'Voce completou todos os treinos!',
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          ...
          <Badge text={`${completedCount}/${totalSets} series`} color={Colors.primary} bgColor={Colors.primary + '20'} />
        </View>
      </View>
      ...
      <ScrollView style={styles.exerciseList} showsVerticalScrollIndicator={false}>
        {workout.exercises.map((we, exerciseIndex) => (
          <View key={exerciseIndex}>
            <View style={[
              styles.exerciseCard,
              exerciseIndex === currentExercise && styles.exerciseCardActive,
              exerciseIndex < currentExercise && styles.exerciseCardDone,
            ]}>
              <View style={styles.exerciseHeader}>
                <View style={[
                  styles.exerciseNumber,
                  exerciseIndex === currentExercise && styles.exerciseNumberActive,
                  exerciseIndex < currentExercise && styles.exerciseNumberDone,
                ]}>
                  {exerciseIndex < currentExercise ? (
                    ...
                    <Text style={[
                      ...
                      exerciseIndex === currentExercise && styles.exerciseNumberTextActive,
                ...
                <View style={styles.exerciseInfo}>
                  <Text style={[
                    styles.exerciseName,
                    exerciseIndex < currentExercise && styles.exerciseNameDone,
                  ]}>{we.exercise.name}</Text>
                  <Text style={styles.exerciseMeta}>{we.exercise.muscle}</Text>
                </View>
                {exerciseIndex === currentExercise && (
                  <Badge text="AGORA" color={Colors.success} bgColor={Colors.successBg} />
                )}
              </View>
                ...
                <>
                  ...
                  {we.sets.map((s, setIndex) => (
                    <View key={setIndex} style={[
                      styles.setItem,
                      setIndex === currentSet && styles.setItemActive,
                      completedSets[exerciseIndex][setIndex] && styles.setItemDone,
                    ]}>
                      <Text style={[
                        styles.setItemNumber,
                        setIndex === currentSet && styles.setItemNumberActive,
                      ]}>{setIndex + 1}</Text>
          ...
          >
            <Text style={styles.mainButtonText}>
              {currentSet < exercise.sets.length - 1
                ? `Proxima Serie (${currentSet + 2}/${exercise.sets.length})`
